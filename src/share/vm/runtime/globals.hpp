...
#define RUNTIME_FLAGS(develop, \
  ...
  develop(bool, GenerateRangeChecks, true,                                  \
          "Generate range checks for array accesses")                       \
                                                                            \
  product_pd(bool, ImplicitNullChecks,                                      \
  ...
  product_pd(bool, PreserveFramePointer,                                    \
             ...
             "and not as a general purpose register.")                      \
                                                                            \
  product(bool, SerializeProfilingData, false,                              \
             "location to record profiling data in a file for later use.")  \
                                                                            \
  product(ccstr, SerializedProfilingLoadLocation, NULL,                     \
             "Location to try to load serialized profiling data from.")     \
                                                                            \
  product(bool, SerializeProfilingDataForCompiledMethodsOnly, false,        \
             "Only serialize profiling data for compiled methods.")         \
                                                                            \
  product(bool, SerializeProfilingDataAtOverflow, false,                    \
             "Serialized profiling data for a method when the method "      \
             "triggers an overflow in the interpreter.")                    \
                                                                            \
  product(bool, ZeroSerializedProfilingMethodCounters, false,               \
             "While parsing saved profiling data, zero out counters.")      \
                                                                            \
  product(bool, ZeroInvocationAndBackedgeCounters, false,                   \
             "Zero the invocation and backedge counters when the vm"        \
             "indicator is tripped the first time.")                        \
                                                                            \
  product(bool, ClearMethodCounters, false,                                 \
             "Clear method counters between iterations.")                   \
                                                                            \
  product(bool, AllocMethodDataOnInvocation, false,                         \
             "Allocate an MDO for a method the first time it is invoked.")  \
                                                                            \
  product(ccstr, SerializedProfilingWriteLocation, NULL,                    \
             "Location to try to write serialized profiling data to.")     \
                                                                            \
  product(bool, LoadSerializedProfilingData, false,                         \
             "Load serialized profiling data structures.")                  \
                                                                            \
  product(bool, LoadProfilingDataOnInvocation, false,                       \
             "Loads serialized profiling data on first method invocation.") \
                                                                            \
  product(bool, SkipSerializedInvokeAndBackedgeCounters, false,             \
             "When loading serialized profiling data, don't load the "      \
             "invocation or backedge counters.")                            \
                                                                            \
  product(bool, PreventMethodCompilation, false,                            \
             "Don't compile methods until the VMIndicator is tripped.")     \
                                                                            \
  product(bool, CompileOnlyMethodsWithLoadedProfile, false,                 \
             "Only send methods that have had profiling data loaded to be"  \
             "compiled")                                                    \
                                                                            \
  product(ccstr, CompilableMethodsListLocation, NULL,                       \
             "Location of a file that contains a list of methods. The "     \
             "methods listed therein are the only methods that will be "    \
             "compiled.")                                                   \
                                                                            \
  product(bool, PrintCompilationFullMethodNames, false,                     \
             "Print full method names when printing method compilation"     \
             " info.")                                                      \
                                                                            \
  product(bool, DumpAndReloadProfilingData, false,                          \
             "Dump and reload profiling data when the interpreter hook is " \
             "triggered the first time.")                                   \
                                                                            \
  product(bool, DisableProfileInterpreter, false,                           \
             "Disable interpreter profiling without the other side-effects "\
             " of -XX:-ProfileInterpreter.")                                \
                                                                            \
  product(bool, IgnoreMethodMaturity, false,                                \
             "Assume that methods are always mature.")                      \
                                                                            \
  product(bool, ZeroLoadedInvocationAndBackedgeCounters, false,             \
             "Zero invocation and backedge counters from loaded profiling " \
             "data.")                                                       \
                                                                            \
  product(ccstr, ProfilingDataPathsLocation, NULL,                          \
             "Location to write data about profiling driven decisions in"   \
             "C2")                                                          \
                                                                            \
  product(bool, ProfilingDataPathsIncludeMethodNames, false,                \
             "If true, the names of methods will be recorded in the pdps "  \
             "file. This is potentially very expensive memory-wise, so it's"\
             "disabled by default.")                                        \
                                                                            \
  product(bool, QuietDeserializationWarning, false,                         \
             "")                                                            \
                                                                            \
  product(bool, StartWithCompiliationDisabled, false,                       \
             "Start with compilation initially disabled. It will be re-"    \
             "enabled at the vm-indicator.")                                \
                                                                            \
  product(bool, WaitAtVmIndicatorForCompilerQueue, false,                   \
             "Waits at the VmIndicator method the first time its reached "  \
             "until the (c2) compiler queue is empty.")                     \
                                                                            \
  product(bool, SortC2CompilerQueue, false,                                 \
             "Sorts the tasks in the C2 compiler queue. Sorting happens at "\
             "the vm-indicator.")                                           \
                                                                            \
  product(bool, RandomizeProfilingDecisions, false,                         \
             "Randomizes the profiling-based decisions C2 makes.")          \
                                                                            \
  product(intx, RandomizeProfilingDecisionsPercent, 100,                   \
             "When randomizing decisions, what percentage of the time "     \
             "should the prof-influenced decision be taken.")               \
                                                                            \
  product(ccstr, MethodMaxCounterFileLocation, NULL,                        \
             "Location of a file to read method counter data from (for "    \
             "controlling method profiling bounds).")                       \
                                                                            \
  product(intx, ProfileInterpreterBoundsRange, -1,                          \
             "Setting this to a positive number enables interpreter "       \
             "profiling that are independent from CompileThreshold. The "   \
             "value determines the size range between the bounds. ")        \
                                                                            \
  product(intx, ProfileInterpreterBoundsType, -1,                           \
             "0: 0-CompileThreshold\n"                                      \
             "1: The CompileThreshold middle invocations of the max value\n"\
             "2: The CompileThreshold last invocations of the max value")   \
                                                                            \
  product(bool, IgnoreCompilableMethodsList, false,                         \
             "Use CompilableMethodsListLocation only for ordering.")        \
                                                                            \
  product(bool, PrintPerMethodBytecodesInlinedCounts, false,                \
             "")                                                            \
                                                                            \
  product(bool, RandomizeSerializedTraps, false,                            \
             "")                                                            \
                                                                            \
  product(bool, RandomizeSerializedProfData, false,                         \
             "")                                                            \
                                                                            \
  product(bool, RandomizeSerializedProfDataMore, false,                     \
             "")                                                            \
                                                                            \
  product(intx, DisableOpt0, 0, "")                                         \
  product(intx, DisableOpt1, 0, "")                                         \
  product(intx, DisableOpt2, 0, "")                                         \
  product(intx, DisableOpt3, 0, "")                                         \
  product(intx, DisableOpt4, 0, "")                                         \
  product(intx, DisableOpt5, 0, "")                                         \
  product(intx, DisableOpt6, 0, "")                                         \
  product(intx, DisableOpt7, 0, "")                                         \
  product(intx, DisableOpt8, 0, "")                                         \
  product(intx, DisableOpt9, 0, "")                                         \
  product(intx, DisableOpt10, 0, "")                                        \
  product(intx, DisableOpt11, 0, "")                                        \
  product(intx, DisableOpt12, 0, "")                                        \
  product(intx, DisableOpt13, 0, "")                                        \
  product(intx, DisableOpt14, 0, "")                                        \
  product(intx, DisableOpt15, 0, "")                                        \
  product(intx, DisableOpt16, 0, "")                                        \
  product(intx, DisableOpt17, 0, "")                                        \
  product(intx, DisableOpt18, 0, "")                                        \
  product(intx, DisableOpt19, 0, "")                                        \
  product(intx, DisableOpt20, 0, "")                                        \
  product(intx, DisableOpt21, 0, "")                                        \
  product(intx, DisableOpt22, 0, "")                                        \
  product(intx, DisableOpt23, 0, "")                                        \
  product(intx, DisableOpt24, 0, "")                                        \
  product(intx, DisableOpt25, 0, "")                                        \
  product(intx, DisableOpt26, 0, "")                                        \
  product(intx, DisableOpt27, 0, "")                                        \
  product(intx, DisableOpt28, 0, "")                                        \
  product(intx, DisableOpt29, 0, "")                                        \
  product(intx, DisableOpt30, 0, "")                                        \
  product(intx, DisableOpt31, 0, "")                                        \
  product(intx, DisableOpt32, 0, "")                                        \
  product(intx, DisableOpt33, 0, "")                                        \
  product(intx, DisableOpt34, 0, "")                                        \
  product(intx, DisableOpt35, 0, "")                                        \
  product(intx, DisableOpt36, 0, "")                                        \
  product(intx, DisableOpt37, 0, "")                                        \
  product(intx, DisableOpt38, 0, "")                                        \
  product(intx, DisableOpt39, 0, "")                                        \
  product(intx, DisableOpt40, 0, "")                                        \
  product(intx, DisableOpt41, 0, "")                                        \
  product(intx, DisableOpt42, 0, "")                                        \
  product(intx, DisableOpt43, 0, "")                                        \
  product(intx, DisableOpt44, 0, "")                                        \
  product(intx, DisableOpt45, 0, "")                                        \
  product(intx, DisableOpt46, 0, "")                                        \
  product(intx, DisableOpt47, 0, "")                                        \
  product(intx, DisableOpt48, 0, "")                                        \
  product(intx, DisableOpt49, 0, "")                                        \
  product(intx, DisableOpt50, 0, "")                                        \
  product(intx, DisableOpt51, 0, "")                                        \
  product(intx, DisableOpt52, 0, "")                                        \
  product(intx, DisableOpt53, 0, "")                                        \
  product(intx, DisableOpt54, 0, "")                                        \
  product(intx, DisableOpt55, 0, "")                                        \
  product(intx, DisableOpt56, 0, "")                                        \
  product(intx, DisableOpt57, 0, "")                                        \
  product(intx, DisableOpt58, 0, "")                                        \
  product(intx, DisableOpt59, 0, "")                                        \
  product(intx, DisableOpt60, 0, "")                                        \
  product(intx, DisableOpt61, 0, "")                                        \
  product(intx, DisableOpt62, 0, "")                                        \
  product(intx, DisableOpt63, 0, "")                                        \
  /*product(intx, DisableOpt64, 0, "")*/                                        \
  /*product(intx, DisableOpt65, 0, "")*/                                        \
  /*product(intx, DisableOpt66, 0, "")*/                                        \
  /*product(intx, DisableOpt67, 0, "")*/                                        \
  /*product(intx, DisableOpt68, 0, "")*/                                        \
  /*product(intx, DisableOpt69, 0, "")*/                                        \
  product(intx, DisableOptTooManyTraps, 0, "")                              \
  product(intx, DisableOptTooManyRecompiles, 0, "")                         \
                                                                            \
  product(bool, XXX_AggressiveCoalesce, true, "")                           \
  product(bool, XXX_CondConstProp, true, "")                                \
  product(bool, XXX_IterGVN, true, "")                                      \
  product(bool, XXX_LoopPeeling, true, "")                                  \
  product(bool, XXX_OptoGCM, true, "")                                      \
  product(bool, XXX_OptoNullCheck, true, "")                                \
  product(bool, XXX_OptoRegAlloc, true, "")                                 \
  product(bool, XXX_ParseGVN, true, "")                                     \
  product(bool, XXX_PeepholeRemoveCopies, true, "")                         \
  product(intx, XXX_AWWReduceRegAlloc, 0,                                   \
          "0 - Normal"                                                      \
          "1 - Half"                                                        \
          "2 - Quarter"                                                     \
          "3 - Minimal")                                                    \
...
#define CHECK_OPT_BOOL(N, BCI, METHOD, DEFAULT, T, F) \
    Compile::current()->method()->get_Method()->set_pdp(N, BCI, METHOD, DisableOpt##N,\
        (DisableOpt##N == 0 || DisableOpt##N == 3) ? (DEFAULT) : \
        (DisableOpt##N == 1) ? (T) : \
        (DisableOpt##N == 2) ? (F) : \
        (*((char*)0x1) && "Invalid value for DisableOpt" #N) \
    )

#define CHECK_OPT_VAL(N, BCI, METHOD, TYPE, DEFAULT, NOP) \
    Compile::current()->method()->get_Method()->set_pdp(N, BCI, METHOD, DisableOpt##N, \
        (DisableOpt##N == 0 || DisableOpt##N == 3) ? (DEFAULT) : \
        (DisableOpt##N == 1) ? NOP : \
        (typeof(DEFAULT))(*((char*)0x1) && "Invalid value for DisableOpt" #N) \
    )

#define CHECK_OPT_VAL_2(N, BCI, METHOD, TYPE, DEFAULT, VAL_T, VAL_F) \
    Compile::current()->method()->get_Method()->set_pdp(N, BCI, METHOD, DisableOpt##N, \
        (DisableOpt##N == 0 || DisableOpt##N == 3) ? (DEFAULT) : \
        (DisableOpt##N == 1) ? (VAL_T) : \
        (DisableOpt##N == 2) ? (VAL_F) : \
        (typeof(DEFAULT))(*((char*)0x1) && "Invalid value for DisableOpt" #N) \
    )

#define CHECK_OPT_KLASS(N, BCI, METHOD, DEFAULT) \
    Compile::current()->method()->get_Method()->set_pdp(N, BCI, METHOD, DisableOpt##N, \
        (DisableOpt##N == 0 || DisableOpt##N == 3) ? (DEFAULT) : \
        (DisableOpt##N == 1) ? (NULL) : \
        (ciKlass*)(*((char*)0x1) && "Invalid value for DisableOpt" #N) \
    )
