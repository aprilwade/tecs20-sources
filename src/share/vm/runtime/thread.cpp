...
CompilerThread* CompilerThread::_first_ct = NULL;
...
CompilerThread::CompilerThread(CompileQueue* queue,
  ...
  _compiler = NULL;
  _next_ct = NULL;
  _pause_comp_monitor = new Monitor(Mutex::leaf, "Pause Compilation Lock");
  _pause_comp = StartWithCompiliationDisabled;
...
void CompilerThread::unpause() {

  CompilerThread *thread = CompilerThread::_first_ct;
  while(thread != NULL) {
    thread->_pause_comp = false;
    {
      MutexLocker lock(thread->_pause_comp_monitor);
      thread->_pause_comp_monitor->notify();
    }
    thread = thread->_next_ct;
  }
}
...
jint Threads::create_vm(JavaVMInitArgs* args, bool* canTryAgain) {
  ...
  Threads::parse_compilable_methods_list();
  if(MethodMaxCounterFileLocation) {
    Method::build_max_counter_db(MethodMaxCounterFileLocation);
  }
  if(SerializedProfilingLoadLocation) {
    Method::build_prof_data_db(SerializedProfilingLoadLocation);
  }
  if(SerializeProfilingDataAtOverflow) {
    if(SerializedProfilingWriteLocation == NULL) {
      tty->print_cr("Warning: SerializeProfilingDataAtOverflow was set without "
                    "SerializedProfileAndWriteLocation being set");
    } else {
      ProfilingDataSerializer::shared()->open(SerializedProfilingWriteLocation);
    }
  }
...
void Thread::print_and_update_timing() {
  double old_user = this->_user_time;
  double old_sys  = this->_sys_time;

  this->timing(&this->_user_time, &this->_sys_time);

  double user_diff = this->_user_time - old_user;
  double sys_diff  = this->_sys_time  - old_sys;

  tty->print_cr("Thread (%s) total:%f user:%f system:%f", this->name(),
                  user_diff + sys_diff, user_diff, sys_diff);
}


pmap<pstring, uint> *Threads::_compilable_methods = NULL;
uint Threads::max_method_priority;
void Threads::parse_compilable_methods_list() {
  if(CompilableMethodsListLocation == NULL) {
    return;
  }

  FILE *f = fopen(CompilableMethodsListLocation, "r");
  if(f == NULL) {
    tty->print_raw_cr("Warning: Failed to open CompilableMethodsListLocation.");
    return;
  }
  ...
  _compilable_methods = new (&nont) pmap<pstring, uint>;
  uint i = 1;
  while(1) {
  ...
  max_method_priority = i;
}

uint Threads::compilable_methods_contains_priority(Method *m) {

  if(_compilable_methods == NULL) {
    return 1;
  }
  char buf[2048];
  m->name_and_sig_as_C_string(buf, sizeof(buf));
  pmap<pstring, uint>::iterator it = _compilable_methods->find(buf);
  if(it == _compilable_methods->end()) {
    return 0;
  }
  return it->second;
}
