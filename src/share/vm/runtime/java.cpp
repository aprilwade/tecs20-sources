...
void write_serialized_profiing_data(const char *location)
{

  ResourceMark rm;
  HandleMark hm;

  collected_invoked_methods = new GrowableArray<Method*>(8192);
  SystemDictionary::methods_do(collect_invoked_methods);

  ProfilingDataSerializer pds;
  pds.open(location);

  for(int i = 0; i < collected_invoked_methods->length(); i++) {
    Method *m = collected_invoked_methods->at(i);
    if(SerializeProfilingDataForCompiledMethodsOnly &&
       !m->has_compiled_code()) {
      continue;
    }
    pds.serialize_method(m);
  }

  pds.close();
}

void write_pdps(const char *location)
{
  static FILE *f;
  f = fopen(location, "w");
  ...
  static bool first;
  first = true;
  struct PDPMethodsVisitor {
    static void visit_method(Method *m) {
      m->dump_pdps(f, &first);
    }
  };
  fputs("{\n", f);
  SystemDictionary::methods_do(PDPMethodsVisitor::visit_method);
  fputs("\n}\n", f);
...
#endif
void print_method_counter_facts();
...
void before_exit(JavaThread* thread) {
  ...
  if(SerializeProfilingData) {
    if(SerializedProfilingWriteLocation == NULL) {
      tty->print_cr("Warning: SerializeProfilingData was set without "
                    "SerializedProfileAndWriteLocation being set");
    } else {
      write_serialized_profiing_data(SerializedProfilingWriteLocation);
    }
  }
  if(ProfilingDataSerializer::shared()->is_open()) {
    ProfilingDataSerializer::shared()->close();
  }

  if(ProfilingDataPathsLocation) {
    write_pdps(ProfilingDataPathsLocation);
  }
