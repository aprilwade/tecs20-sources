...
inline void CompilerThread::check_paused() {
  if(this->_pause_comp) {
    MonitorLockerEx lock(this->_pause_comp_monitor);
    while(this->_pause_comp) {
      lock.wait();
    }
  }
}

inline void CompilerThread::init_ct_pointers() {
  MutexLocker lock(CompileThread_lock, this);
  if(CompilerThread::_first_ct == NULL) {
    CompilerThread::_first_ct = this;
  } else {
    CompilerThread *next_thread = CompilerThread::_first_ct;
    while(next_thread->_next_ct != NULL) {
      next_thread = next_thread->_next_ct;
    }
    next_thread->_next_ct = this;
  }
}
