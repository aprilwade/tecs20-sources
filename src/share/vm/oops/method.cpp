...
Method::Method(ConstMethod* xconst, AccessFlags access_flags) {
  ...
  _has_loaded_profiling_data = false;
  _cmf_state = cmf_unchecked;//cmf_cannot_compile;
  _comp_priority = 0;

  NOT_PRODUCT(set_compiled_invocation_count(0);)
  _bytecodes_inlined = 0;
}
...
static void classes_do_func(Klass *klass, ClassLoaderData *cld) {

  const char *klass_cname = klass->name()->as_C_string();
  const char *loader_cname = cld->loader_name();
  pstring name(klass_cname);
  name += ';';
  name += loader_cname;

  SerializatiedKlassList &kl = SerializatiedKlassList::klasses_with_name(name);

  kl.append(klass);
}

void Method::load_method_data_from_file(const char *location) {
  /*
  ...
  */
  ...
  if(f == NULL) {
    tty->print_raw_cr("Warning: Couldn't open profiling data");
    return;
  }

  Thread *current_thread = Thread::current();
  SystemDictionary::classes_do(classes_do_func);

  tty->print("Starting json parse\n");

  char json_buffer[65536];
  rapidjson::FileReadStream is(f, json_buffer, sizeof(json_buffer));

  rapidjson::Document doc;
  doc.ParseStream(is);

  assert(doc.IsArray(), "Profiling data top level must be an array");
  rapidjson::Value::ConstValueIterator it_end = doc.End();
  for(rapidjson::Value::ConstValueIterator it = doc.Begin(); it != it_end; ++it) {
    const rapidjson::Value &obj = (*it);
    assert(obj["klass"].IsString(), "Hmm");


    unsigned hash;
    pstring klass_str = obj["klass"].GetString();
    klass_str += ';';
    klass_str += obj["loader"].GetString();

    const char *method_str = obj["name"].GetString();
    Symbol *method_name = SymbolTable::lookup_only(method_str, strlen(method_str),
                                                   hash);

    const char *sig_str = obj["sig"].GetString();
    Symbol *sig_name = SymbolTable::lookup_only(sig_str, strlen(sig_str), hash);


    if(method_name == NULL || sig_name == NULL)
      continue;

    // Using SystemDictionary to look up a klass based on its name only really
    // works when we know its class loader. So, we're cheating a bit by making
    // our using map.
    SerializatiedKlassList &kl = SerializatiedKlassList::klasses_with_name(klass_str);
    if(kl.size() == 0) {
      if(!QuietDeserializationWarning) {
        tty->print("Failed to find class %s\n", klass_str.c_str());
      }
      continue;
    }
    ...
    for(size_t i = 0; i < kl.size(); i++) {
      ...
      Method *m = k->lookup_method(method_name, sig_name);
      if(m == NULL) {
        tty->print("Found class %s but failed to find method %s\n", klass_str.c_str(), method_str);
        continue;
      }

      if(obj["mco"].MemberCount()) {
        m->_method_counters = MethodCounters::deserialize_from_json(m, obj["mco"], current_thread);
      }
      if(obj["mdo"].MemberCount()) {
        m->_method_data = MethodData::deserialize_from_json(m, obj["mdo"], current_thread);
      }
      m->_has_loaded_profiling_data = true;
    }
  }

  tty->print("Finished json parse\n");
...
void Method::print_full_name(outputStream *st) {
  st->print_raw(this->name_and_sig_as_C_string());
}

// Exposed so field engineers can debug VM
void Method::print_short_name(outputStream* st) {
  if(PrintCompilationFullMethodNames) {
    print_full_name(st);
    return;
  }
  ResourceMark rm;
...
void Method::interp_alloc_mdo(JavaThread *THREAD) {
  ThreadInVMfromJava tivmfj(THREAD);
  frame fr = THREAD->last_frame();
  assert(fr.is_interpreted_frame(), "must come from interpreter");
  methodHandle method(THREAD, fr.interpreter_frame_method());
  //assert(method->method_data() == NULL, "method must not yet have an mdo");
  Method::build_interpreter_method_data(method, CHECK_AND_CLEAR);
}

Method::prof_data_db_t* Method::_prof_data_db;
rapidjson::Document *Method::_prof_data_doc;
uint32_t Method::_loaded_prof_count;
uint32_t Method::_attempted_lookup_prof_count;

void Method::build_prof_data_db(const char *location) {
  ...
  tty->print("Starting json db parse\n");

  char json_buffer[65536];
  rapidjson::FileReadStream is(f, json_buffer, sizeof(json_buffer));

  _prof_data_doc = new (&nont) rapidjson::Document;
  _prof_data_doc->ParseStream(is);

  _prof_data_db = new (&nont) prof_data_db_t;

  rapidjson::Value::ConstValueIterator it_end = _prof_data_doc->End();
  for(rapidjson::Value::ConstValueIterator it = _prof_data_doc->Begin(); it != it_end; ++it) {
    const rapidjson::Value &obj = (*it);
    assert(obj["klass"].IsString(), "Hmm");

    pstring str(obj["loader"].GetString());
    str += "::";
    str += obj["klass"].GetString();
    str += "::";
    str += obj["name"].GetString();
    str += "::";
    str += obj["sig"].GetString();

    _prof_data_db->insert(std::pair<pstring, const rapidjson::Value&>(str, obj));
  }

  tty->print("Finised json db parse\n");
}

const rapidjson::Value* Method::try_lookup_prof_data() {

  if(!Method::_prof_data_db) {
    return NULL;
  }

  _attempted_lookup_prof_count++;
  ...
  Symbol *method_sig = this->signature();
  method_sig->as_C_string(method_sig_buf, sizeof(method_sig_buf));

  char klass_name_buf[1024];
  Symbol *klass_name = this->klass_name();
  klass_name->as_C_string(klass_name_buf, sizeof(klass_name_buf));

  const char *loader_name = this->method_holder()->class_loader_data()->loader_name();

  snprintf(buffer, sizeof(buffer), "%s::%s::%s::%s",
          loader_name, klass_name_buf, method_name_buf, method_sig_buf);

  prof_data_db_t::iterator it = _prof_data_db->find(buffer);
  if(it == _prof_data_db->end()) {
    if(!QuietDeserializationWarning) {
      tty->print_cr("Failed to load prof data for %s::%s",
                      klass_name_buf, method_name_buf);
    }
    return NULL;
  }

  // XXX: Incrementing this here is a little disingenuous given its name, but
  //      it does capture the correct behavior.
  _loaded_prof_count++;

  return &it->second;
}

void Method::dump_pdps(FILE *f, bool *first) {
  //
  if(this->_pdps.size() == 0) {
    return;
  }
  ...
  fprintf(f, "    \"%s\": [\n", this->name_and_sig_as_C_string());
  for(uint i = 0; i < this->_pdps.size(); i++) {
    if(i != 0) {
      fputs(",\n", f);
    }
    PDP pdp = this->_pdps[i];
    if(pdp.type == pdp_type_str) {
      fprintf(f, "        [\"%s\", %d, %d, \"%s\"]", pdp.str, pdp.bci, pdp.index, (char*)pdp.val);
    } else if(pdp.type == pdp_type_bool) {
      fprintf(f, "        [\"%s\", %d, %d, %s]", pdp.str, pdp.bci, pdp.index,
                                                 pdp.val ? "true" : "false");
    } else {
      fprintf(f, "        [\"%s\", %d, %d]", pdp.str, pdp.bci, pdp.index);
    }
  }
  fputs("\n    ]", f);
}
...
pmap<pstring, uint> *Method::_max_counter_db = NULL;

void Method::build_max_counter_db(const char *location) {
  if(location == NULL) {
    return;
  }
  ...
  FILE *f = fopen(location, "r");
  if(f == NULL) {
    tty->print_raw_cr("Warning: Couldn't open max counter data");
    return;
  }

  tty->print("Starting max counter db parse\n");
  ...
  _max_counter_db = new (&nont) pmap<pstring, uint>;

  rapidjson::Value::ConstMemberIterator it_end = doc.MemberEnd();
  for(rapidjson::Value::ConstMemberIterator it = doc.MemberBegin(); it != it_end; ++it) {

    _max_counter_db->insert(std::pair<pstring, uint>(it->name.GetString(), it->value.GetUint()));
  }

  tty->print("Finised max counter db parse %lu\n", _max_counter_db->size());
}

uint Method::lookup_max_counter() {
  if(_max_counter_db == NULL) {
  ...
  pmap<pstring, uint>::iterator it = _max_counter_db->find(buffer);
  if(it == _max_counter_db->end()) {
    return 0;
  }
  return it->second;
}

void print_pdp_vec_inner(void *vec_) {
  pvector<Method::PDP> *vec = (pvector<Method::PDP> *)vec_;
  for(uint i = 0; i < vec->size(); i++) {
    Method::PDP &pdp = vec->at(i);
    tty->print_cr(
        "PDP:\n"
        "    index: %d\n"
        "    bci: %d\n"
        "    val: %lu\n"
        "    str: %s\n",
        pdp.index, pdp.bci, pdp.val, pdp.str
    );
  }
}

extern "C" void print_pdp_vec(void *vec) {
  print_pdp_vec_inner(vec);
}
