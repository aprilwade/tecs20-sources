...
void MethodCounters::setup_profiling_bounds(methodHandle mh) {
  uint max_counter = mh->lookup_max_counter();

  uint upper;
  uint lower;
  if(max_counter < ProfileInterpreterBoundsRange || ProfileInterpreterBoundsType == 0) {
    upper = ProfileInterpreterBoundsRange;
    lower = 0;
  } else if(ProfileInterpreterBoundsType == 1) {
    upper = max_counter / 2 + ProfileInterpreterBoundsRange / 2;
    lower = max_counter / 2 - ProfileInterpreterBoundsRange / 2;
  } else if(ProfileInterpreterBoundsType == 2) {
    upper = max_counter;
    lower = max_counter - ProfileInterpreterBoundsRange;
  }

  _profiling_upper_bound = upper << InvocationCounter::count_shift;
  _profiling_lower_bound = lower << InvocationCounter::count_shift;
}
