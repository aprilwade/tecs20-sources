...
class Method : public Metadata {
 ...
 private:
  ...
  // Is this method annotated with "VMIndicator"?
  int _has_vm_indicator;
 ...
 public:
  ...
  MethodData* method_data_old() const              {
    return _method_data_old;
  }
  ...
  static ByteSize vm_indicator_offset() {
    return byte_offset_of(Method, _has_vm_indicator);
  }
 ...
 public:
  ...
  // Printing
  void print_full_name(outputStream* st = tty);
  ...
  int has_vm_indicator() const {
    return _has_vm_indicator;
  }
  void set_has_vm_indicator(int i) {
    _has_vm_indicator = i;
  }
  static void load_method_data_from_file(const char *location);

  bool _has_loaded_profiling_data;
  static ByteSize has_loaded_profiling_data_offset()
  { return byte_offset_of(Method, _has_loaded_profiling_data); }
  ...
  CMFState cmf_state()             { return _cmf_state; }
  void set_cmf_state(CMFState st)  { _cmf_state = st; }
  static ByteSize cmf_state_offset()
  { return byte_offset_of(Method, _cmf_state); }


 public:
  const rapidjson::Value* try_lookup_prof_data();
  static void build_prof_data_db(const char *location);
  static uint32_t loaded_prof_count() { return _loaded_prof_count; }
  static uint32_t attempted_lookup_prof_count() { return _attempted_lookup_prof_count; }

 private:
  // TODO: Use a c-string based structure instead. It should be faster.
  typedef pmap<pstring, const rapidjson::Value&> prof_data_db_t;
  static prof_data_db_t* _prof_data_db;
  static rapidjson::Document *_prof_data_doc;

  static uint32_t _loaded_prof_count;
  static uint32_t _attempted_lookup_prof_count;
 ...
 public:
  static void interp_alloc_mdo(JavaThread *thread);


  template<typename T>
  T set_pdp(int index, int bci, const ciMethod *method, int flag, T val);
  ciKlass* set_pdp_klass(int index, int bci, ciKlass* val, const ciMethod *method);
  const char *cimethod_name(const ciMethod *method);

  void dump_pdps(FILE *f, bool *first);
 private:
  enum PDPType {
    pdp_type_int,
    pdp_type_float,
  ...
  struct PDP {
    ...
    PDP(int i, int b, uintptr_t v, const char *s, PDPType t)
        : index(i), bci(b), val(v), str(s), type(t)
    { }
  };
  pvector<PDP> _pdps;

 private:
  typedef std::map<pstring, uint, std::less<pstring>,
                   malloc_allocator<std::pair<pstring, uint> > >
            priority_map_t;
  static priority_map_t _priority_map;

  uint _comp_priority;
 public:
  uint comp_priority() { return _comp_priority; }
  void set_comp_priority(uint p) { _comp_priority = p; }



  static pmap<pstring, uint> *_max_counter_db;
  static void build_max_counter_db(const char *location);
  uint lookup_max_counter();

  friend void print_pdp_vec_inner(void *vec);


 private:
  volatile jlong _bytecodes_inlined;
 public:
  jlong bytecodes_inlined() { return _bytecodes_inlined; }
  void incr_bytecodes_inlined(jlong i) {
    Atomic::add(i, &_bytecodes_inlined);
  }
};
...
// Use a 32-bit value so we can perform this check using only integer operations,
// reducing the risk of percision loss related issues
#define CHECK_RandomizeProfilingDecisionsPercent() \
    ((uint32_t)os::random() % 100 < RandomizeProfilingDecisionsPercent)

template<typename T>
inline T Method::set_pdp(int index, int bci, const ciMethod *method, int flag, T val) {
  if(flag == 3) {
    if(!CHECK_RandomizeProfilingDecisionsPercent()) {
      val = 0;
    }
  }
  if(index >= 0) {
    this->_pdps.push_back(
        PDP(index, bci, (uintptr_t)val, Method::cimethod_name(method), pdp_type_int));
  } else {
    this->_pdps.push_back(
        PDP(index, bci, (uintptr_t)val, Method::cimethod_name(method), pdp_type_int));
  }
  if(!RandomizeProfilingDecisions) {
    return val;
  } else {
    return os::random();
  }
...
// TODO: We'd like better randomization for floating point values, but the only
//       one that currently exist is [0.0, 1.0], so that's range we're
//       randomizing.
template<>
inline double Method::set_pdp<double>(int index, int bci, const ciMethod *method,
                                      int flag, double val) {
  if(flag == 3) {
    if(!CHECK_RandomizeProfilingDecisionsPercent()) {
      val = 0;
  ...
  if(index >= 0) {
    this->_pdps.push_back(
            PDP(index, bci, (uintptr_t)val, Method::cimethod_name(method), pdp_type_float));
  } else {
    this->_pdps.push_back(
            PDP(index, bci, (uintptr_t)val, Method::cimethod_name(method), pdp_type_float));
  }
  if(!RandomizeProfilingDecisions) {
    ...
    return (double)(uint)os::random() / INT_MAX;
  }
}

template<>
inline float Method::set_pdp<float>(int index, int bci, const ciMethod *method,
                                    int flag, float val) {
  return this->set_pdp<double>(index, bci, method, flag, val);
}
template<>
inline bool Method::set_pdp<bool>(int index, int bci, const ciMethod *method,
                                  int flag, bool val) {
  if(flag == 3) {
    if(!CHECK_RandomizeProfilingDecisionsPercent()) {
      val = !val;
  ...
  this->_pdps.push_back(
          PDP(index, bci, (uintptr_t)val, Method::cimethod_name(method), pdp_type_bool));
  if(!RandomizeProfilingDecisions) {
    return val;
  } else {
    return os::random() % 2;
...
template<>
inline ciKlass* Method::set_pdp<ciKlass*>(int index, int bci, const ciMethod *method,
                                          int flag, ciKlass* val) {
  if(flag == 3) {
    if(!CHECK_RandomizeProfilingDecisionsPercent()) {
      val = NULL;
    }
  }
  ciKlass *k = this->set_pdp_klass(index, bci, val, method);
  if(!RandomizeProfilingDecisions) {
    return val;
  } else {
    return (os::random() % 2) ? val : NULL;
  }
}
