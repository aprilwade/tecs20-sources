...
// If true, randomize!
#define CHECK_RandomizeSerializedProfData() \
    (RandomizeSerializedProfData && ( \
...
public:
  ...
  void unset_flag_at(int flag_number) {
    assert(flag_number < flag_limit, "oob");
    _header._struct._flags &= ~(0x1 << flag_number);
  }
  ...
  bool load_serialized_header(intptr_t value) {
    union header_union new_header = *(union header_union*)&value;
    if(_header._struct._bci != new_header._struct._bci ||
       _header._struct._tag != new_header._struct._tag) {
      return false;
    }

    if(!CHECK_RandomizeSerializedProfData()) {
        _header._struct._flags = new_header._struct._flags;
    } else {
        _header._struct._flags = os::random();
    }
    return true;
  }
};
...
protected:
  ...
  void unset_flag_at(int flag_number) {
    data()->unset_flag_at(flag_number);
  }
  bool flag_at(int flag_number) const {
...
public:
  ...
  virtual void serialize_to_file(const Indenter& indent, FILE *f) const {
    ShouldNotReachHere();
  }
  virtual bool deserialize_from_json(const rapidjson::Value &json) {
    ShouldNotReachHere();
    return false;
  }
...
public:
  ...
  void unset_null_seen()  { unset_flag_at(null_seen_flag); }
...
public:
  ...
  void serialize_to_file_shared(const Indenter& indent, FILE *f) const;
  virtual const char *class_name() const {
    return "ReceiverTypeData";
  }
  virtual bool deserialize_from_json(const rapidjson::Value &json);
};
...
protected:
  ...
  void set_default_count(uint count) {
    array_set_int_at(default_count_off_set, count);
  }
  void set_default_displacement(int displacement) {
    array_set_int_at(default_disaplacement_off_set, displacement);
  }
  void set_count_at(int index, uint count) {
...
public:
  ...
  static Serializer<MethodData> serializer();
  static MethodData *deserialize_from_json(Method *method, const rapidjson::Value &json, TRAPS);
  void deserialize_from_json(const rapidjson::Value &json);
};
