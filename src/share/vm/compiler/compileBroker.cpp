...
int CompileBroker::_sum_bytecodes_inlined       = 0;
...
CompileTask* CompileQueue::get() {
  ...
  /*{
    ResourceMark rm;
    const char *name = task->method()->name_and_sig_as_C_string();
    tty->print_cr("%s", name);
  }*/
...
void CompileBroker::collect_statistics(CompilerThread* thread, elapsedTimer time, CompileTask* task) {
  ...
  if (!success) {
    ...
    _sum_bytecodes_inlined += task->num_inlined_bytecodes();
    task->method()->incr_bytecodes_inlined(task->num_inlined_bytecodes());
...
void CompileQueue::sort_tasks_by_priority() {
  tty->print_cr("Starting sort");

  plist<CompileTask*> list;
  CompileTask *task = this->_last;
  while(task != NULL) {
    list.push_front(task);

    CompileTask *next_task = task->prev();
    task->set_next(NULL);
    task->set_prev(NULL);
    task = next_task;
  }
  ...
  struct CTaskCmp {
    static bool cmp(CompileTask *a, CompileTask *b) {
      return a->method()->comp_priority() < b->method()->comp_priority();
    }
  };
  list.sort(CTaskCmp::cmp);

  plist<CompileTask*>::iterator it = list.begin();
  plist<CompileTask*>::iterator it_end = list.end();
  ...
  for(; it != it_end; it++) {
    (*it)->set_prev(this->_last);
    this->_last->set_next(*it);
    this->_last = *it;
  }

  tty->print_cr("Finished sort");
}
