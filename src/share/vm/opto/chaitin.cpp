...
void PhaseChaitin::Register_Allocate() {
  ...
  /* mrj spill amount from previous iteration */
  uint prev_spills = 0;
  _spillnum_converged = false;
  ...
  while( spills ) {
    ...
    if ( spills <= prev_spills || _lrg_map.max_lrg_id() > 9999) {
      _spillnum_converged = true;
    }
    prev_spills = spills;
...
OptoReg::Name PhaseChaitin::mrj_get_color( LRG &lrg, int chunk ) {

  if ( XXX_OptoRegAlloc || _spillnum_converged )
  ...
  if ( !( !lrg._fat_proj || lrg.is_multidef() || (lrg._def->outcnt() > 0) ) )
    return bias_color(lrg, chunk);
  ...
  if ( !lrg.is_multidef()
       && (lrg._def->ideal_reg() == 0 || lrg._def->ideal_reg() == Op_RegFlags) )
    return bias_color(lrg, chunk);

  if ( !lrg.is_multidef() && (lrg._def->is_Proj() || lrg._def->is_SpillCopy()) )
  /*if ( !lrg.is_multidef() && lrg._def->is_Proj() )*/
    return bias_color(lrg, chunk);
...
OptoReg::Name PhaseChaitin::choose_color( LRG &lrg, int chunk ) {
  ...
  if( lrg.num_regs() == 1 ||    // Common Case
    ...
    return mrj_get_color(lrg, chunk);
    #if 0
    return bias_color(lrg, chunk);
    #endif
  }
