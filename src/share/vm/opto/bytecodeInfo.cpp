...
bool InlineTree::should_inline(ciMethod* callee_method, ciMethod* caller_method,
  ...
  if ((CHECK_OPT_BOOL(60, caller_bci, caller_method,
                     freq >= InlineFrequencyRatio, false, true)) ||
      CHECK_OPT_BOOL(55, caller_bci, caller_method,
                     call_site_count >= InlineFrequencyCount, false, true) ||
...
bool InlineTree::try_to_inline(ciMethod* callee_method, ciMethod* caller_method,
  ...
  if (callee_method->code_size() > MaxTrivialSize) {
    ...
    if ((!UseInterpreter || CompileTheWorld) &&
      ...
      // XXX ciCallProfile:
    } else if (CHECK_OPT_BOOL(56, caller_bci,
                              caller_method, profile.count() == 0, true, false)) {
...
float InlineTree::compute_callee_frequency( int caller_bci ) const {
  ...
  ciMethod *caller_method = caller_jvms() ? caller_jvms()->method() : NULL;
  float freq = CHECK_OPT_VAL_2(59, caller_bci, caller_method, float,
                               (float)count/(float)invcnt, 0, 1.0);
