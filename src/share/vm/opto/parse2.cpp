...
float Parse::dynamic_branch_prediction(float &cnt, BoolTest::mask btest, Node* test) {
  ...
  if (use_mdo) {
    ...
    taken = CHECK_OPT_VAL_2(50, bci(), jvms()->method(), uint, data->as_JumpData()->taken(), 0, UINT_MAX);
    not_taken = 0;
    if (data->is_BranchData()) {
      not_taken = CHECK_OPT_VAL_2(48, bci(), jvms()->method(), uint, data->as_BranchData()->not_taken(), 0, UINT_MAX);
    }
  ...
  if (target_bci < bci()) {
    if (is_osr_parse()) {    // Could be a hot OSR'd loop; force deopt
      ...
      if (!methodData->is_empty()) {
        ...
        // to the OSR loop.
        if (CHECK_OPT_BOOL(34, bci(), jvms()->method(),
            data->as_BranchData()->taken() +
...
bool Parse::seems_stable_comparison() const {
  if (CHECK_OPT_BOOL(27, bci(), jvms()->method(), C->too_many_traps(method(), bci(), Deoptimization::Reason_unstable_if),
                false, true)) {
    return false;
  }
  return true;
...
void Parse::do_one_bytecode() {
  ...
  case Bytecodes::_goto_w: {
    ...
    int taken = CHECK_OPT_VAL_2(49, bci(), method(), uint, ((ciJumpData*)data)->taken(), 0, UINT_MAX);
    taken = method()->scale_count(taken);
  ...
  handle_if_null:
    ...
    if (!_gvn.type(b)->speculative_maybe_null() &&
        CHECK_OPT_BOOL(28, bci(), jvms()->method(), !too_many_traps(Deoptimization::Reason_speculate_null_check), true, false)) {
      inc_sp(1);
      Node* null_ctl = top();
