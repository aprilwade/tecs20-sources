...
Compile::Compile( ciEnv* ci_env, C2Compiler* compiler, ciMethod* target, int osr_bci,
  ...
  //tty->print_cr("eliminate_boxing: %d", _eliminate_boxing);
...
#define MINIMUM_NODE_HASH  1023
  ...
  { // Scope for timing the parser
    ...
    if (is_osr_compilation()) {
      ...
      if (cg == NULL) {
        float past_uses = CHECK_OPT_VAL_2(58, entry_bci(), method(), int, method()->interpreter_invocation_count(), 0, 0x08FFFFFF);
...
void Compile::Init(int aliaslevel) {
  ...
  set_do_freq_based_layout(BlockLayoutByFrequency && _directive->BlockLayoutByFrequencyOption);
...
bool Compile::too_many_traps(ciMethod* method,
  ...
  if(DisableOptTooManyTraps == 1) {
    return true;
  } else if(DisableOptTooManyTraps == 2) {
    return false;
  }
...
bool Compile::too_many_recompiles(ciMethod* method,
                                  int bci,
                                  Deoptimization::DeoptReason reason) {
  if(DisableOptTooManyRecompiles == 1) {
    return true;
  } else if(DisableOptTooManyRecompiles == 2) {
    return false;
  }
  ciMethodData* md = method->method_data();
  if (md->is_empty()) {
...
void Compile::process_print_inlining() {
  ...
  if (do_print_inlining) {
    ResourceMark rm;
    stringStream ss;
    const char * name = this->method()->get_Method()->name_and_sig_as_C_string();
    //ss.print(" ---- %s\n", name);
    for (int i = 0; i < _print_inlining_list->length(); i++) {
