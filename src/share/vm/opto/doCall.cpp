...
CallGenerator* Compile::call_generator(ciMethod* callee, int vtable_index, bool call_does_dispatch,
  ...
  int site_count = CHECK_OPT_VAL_2(62, bci, jvms->method(), int, profile.count(), 1, 0x08FFFFFF);
  ...
  if (call_does_dispatch && UseTypeProfile && profile.has_receiver(0)) {
    ...
    receiver_count = CHECK_OPT_VAL(32, bci, jvms->method(), int, profile.receiver_count(0), -1);
  }
  ...
  if (allow_inline) {
    ...
    if (!call_does_dispatch) {
      ...
      bool allow_inline   = ci != NULL && CHECK_OPT_BOOL(35, bci, jvms->method(), !ci->is_cold(), true, false);
      bool require_inline = (allow_inline && CHECK_OPT_BOOL(36, bci, jvms->method(), ci->is_hot(), false, true));
      ...
      if (receiver_method != NULL) {
        ...
        if (hit_cg != NULL) {
          ...
          if ((morphism == 1 || (morphism == 2 && next_hit_cg != NULL)) &&
              CHECK_OPT_BOOL(13, jvms->bci(), jvms->method(), !too_many_traps(jvms->method(), jvms->bci(), reason), true, false)
             ) {
