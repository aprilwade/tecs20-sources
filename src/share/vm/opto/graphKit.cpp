...
void GraphKit::builtin_throw(Deoptimization::DeoptReason reason, Node* arg) {
  ...
  if (CHECK_OPT_BOOL(1, bci(), jvms()->method(), treat_throw_as_hot, false, true)
  ...
  if (CHECK_OPT_BOOL(2, bci(), jvms()->method(),
      treat_throw_as_hot
      && (method()->method_data()->trap_recompiled_at(bci(), m)
          || C->too_many_traps(reason)), false, true)) {
...
Node* GraphKit::null_check_common(Node* value, BasicType type,
  ...
  if (null_control != NULL || CHECK_OPT_BOOL(3, bci(), jvms()->method(), too_many_traps(reason), false, true)) {
    // probability is less likely
    ok_prob =  PROB_LIKELY_MAG(3);
  } else if (!assert_null &&
             (ImplicitNullCheckThreshold > 0) &&
             method() != NULL &&
             CHECK_OPT_BOOL(4, bci(), jvms()->method(),
              (method()->method_data()->trap_count(reason)
              >= (uint)ImplicitNullCheckThreshold), false, true)) {
    ok_prob =  PROB_LIKELY_MAG(3);
  }
...
void GraphKit::uncommon_trap(int trap_request,
  ...
  case Deoptimization::Action_reinterpret:
    ...
    if (!keep_exact_action &&
        ...
        CHECK_OPT_BOOL(5, bci(), jvms()->method(), too_many_recompiles(reason), false, true)) {
...
Node* GraphKit::record_profiled_receiver_for_speculation(Node* n) {
  ...
  ciKlass* exact_kls = CHECK_OPT_KLASS(51, bci(), jvms()->method(), profile_has_unique_klass());
  ...
  if (java_bc() == Bytecodes::_checkcast ||
    ...
    bool maybe_null = data == NULL ? true :
                        CHECK_OPT_BOOL(6, bci(), jvms()->method(), data->as_BitData()->null_seen(), false, true);
  }
...
void GraphKit::record_profiled_arguments_for_speculation(ciMethod* dest_method, Bytecodes::Code bc) {
  ...
  for (int j = skip, i = 0; j < nargs && i < TypeProfileArgsLimit; j++) {
    ...
    if (targ->basic_type() == T_OBJECT || targ->basic_type() == T_ARRAY) {
      ...
      bool res = method()->argument_profiled_type(bci(), i, better_type, maybe_null);
      if (res) {
        record_profile_for_speculation(
            argument(j),
            CHECK_OPT_KLASS(52, bci(), jvms()->method(), better_type),
            CHECK_OPT_BOOL(42, bci(), jvms()->method(), maybe_null, false, true)
        );
      }
      i++;
    }
...
void GraphKit::record_profiled_parameters_for_speculation() {
  ...
  for (int i = 0, j = 0; i < method()->arg_size() ; i++) {
    if (_gvn.type(local(i))->isa_oopptr()) {
      bool maybe_null = true;
      ciKlass* better_type = NULL;
      bool res = method()->parameter_profiled_type(j, better_type, maybe_null);
      if (res) {
        record_profile_for_speculation(
            local(i),
            CHECK_OPT_KLASS(53, bci(), jvms()->method(), better_type),
            CHECK_OPT_BOOL(44, bci(), jvms()->method(), maybe_null, false, true)
        );
      }
      j++;
    }
...
void GraphKit::record_profiled_return_for_speculation() {
  ...
  bool maybe_null = true;
  ciKlass* better_type = NULL;
  bool res = method()->return_profiled_type(bci(), better_type, maybe_null);
  if (res) {
    ...
    record_profile_for_speculation(
        stack(sp()-1),
        CHECK_OPT_KLASS(54, bci(), jvms()->method(), better_type),
        CHECK_OPT_BOOL(46, bci(), jvms()->method(), maybe_null, false, true)
    );
  }
}
...
bool GraphKit::seems_never_null(Node* obj, ciProfileData* data, bool& speculating) {
  ...
  if (UncommonNullCast               // Cutout for this technique
      ...
      && !CHECK_OPT_BOOL(7, bci(), jvms()->method(), too_many_traps(reason), false, true)
      ) {
    if (speculating) {
      return true;
    }
    ...
    return CHECK_OPT_BOOL(38, bci(), jvms()->method(), !data->as_BitData()->null_seen(), true, false);
  }
  speculating = false;
  return false;
}
...
Node* GraphKit::maybe_cast_profiled_receiver(Node* not_null_obj,
  ...
  if (CHECK_OPT_BOOL(8, bci(), jvms()->method(), too_many_traps(reason) || too_many_recompiles(reason),
                false, true))
    return NULL;
  ...
  ciKlass* exact_kls = spec_klass == NULL ?
                        CHECK_OPT_KLASS(47, bci(), jvms()->method(), profile_has_unique_klass()) :
                        spec_klass;
...
Node* GraphKit::maybe_cast_profiled_obj(Node* obj,
  ...
  if (type != NULL) {
    ...
    if (CHECK_OPT_BOOL(9, bci(), jvms()->method(), !too_many_traps(null_reason) &&
        !too_many_recompiles(null_reason) &&
        !too_many_traps(class_reason) &&
        !too_many_recompiles(class_reason), true, false)) {
    ...
    if (CHECK_OPT_BOOL(10, bci(), jvms()->method(), !too_many_traps(Deoptimization::Reason_null_assert) &&
...
Node* GraphKit::gen_instanceof(Node* obj, Node* superklass, bool safe_for_replace) {
  ...
  bool never_see_null = (ProfileDynamicTypes  // aggressive use of profile
                         && CHECK_OPT_BOOL(37, bci(), jvms()->method(),
                            seems_never_null(obj, data, speculative_not_null), true, false));
...
Node* GraphKit::gen_checkcast(Node *obj, Node* superklass,
  ...
  bool never_see_null = ((failure_control == NULL)  // regular case only
                         && CHECK_OPT_BOOL(39, bci(), jvms()->method(),
                            seems_never_null(obj, data, speculative_not_null), true, false));
...
Node* GraphKit::new_array(Node* klass_node,     // array klass (maybe variable)
  ...
  if (!layout_is_con && !StressReflectiveCode &&
      CHECK_OPT_BOOL(11, bci(), jvms()->method(), !too_many_traps(Deoptimization::Reason_class_check), true, false)) {
...
void GraphKit::add_predicate_impl(Deoptimization::DeoptReason reason, int nargs) {
  // Too many traps seen?
  if (CHECK_OPT_BOOL(12, bci(), jvms()->method(), too_many_traps(reason), false, true)) {
#ifdef ASSERT
