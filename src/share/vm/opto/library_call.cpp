...
bool LibraryCallKit::inline_native_Class_query(vmIntrinsics::ID id) {
  ...
  bool never_see_null = CHECK_OPT_BOOL(18, bci(), jvms()->method(), !too_many_traps(Deoptimization::Reason_null_check),
                                           true, false);
...
bool LibraryCallKit::inline_Class_cast() {
  ...
  if (CHECK_OPT_BOOL(19, bci(), jvms()->method(), too_many_traps(Deoptimization::Reason_intrinsic), false, true)) {
    return false;
  }
...
bool LibraryCallKit::inline_native_subtype_check() {
  ...
  bool never_see_null = CHECK_OPT_BOOL(20, bci(), jvms()->method(), !too_many_traps(Deoptimization::Reason_null_check),
                                           true, false);
  for (which_arg = 0; which_arg <= 1; which_arg++) {
...
bool LibraryCallKit::inline_native_getLength() {
  if (CHECK_OPT_BOOL(22, bci(), jvms()->method(), too_many_traps(Deoptimization::Reason_intrinsic), false, true))
    return false;
...
bool LibraryCallKit::inline_array_copyOf(bool is_copyOfRange) {
  if (CHECK_OPT_BOOL(23, bci(), jvms()->method(), too_many_traps(Deoptimization::Reason_intrinsic), false, true))
    return false;
  ...
  { PreserveReexecuteState preexecs(this);
    ...
    if (!stopped()) {
      ...
      if (CHECK_OPT_BOOL(24, bci(), jvms()->method(), !too_many_traps(Deoptimization::Reason_class_check), true, false)) {
...
JVMState* LibraryCallKit::arraycopy_restore_alloc_state(AllocateArrayNode* alloc, int& saved_reexecute_sp) {
  if (alloc != NULL) {
    ...
    if (CHECK_OPT_BOOL(25, bci(), jvms()->method(),
          !C->too_many_traps(trap_method, trap_bci, Deoptimization::Reason_intrinsic) &
...
bool LibraryCallKit::inline_arraycopy() {
  ...
  if (CHECK_OPT_BOOL(26, bci(), jvms()->method(),
                !C->too_many_traps(trap_method, trap_bci, Deoptimization::Reason_intrinsic),
                true, false) &&
      can_emit_guards &&
