...
static void clear_method_counters() {

  class Inner {
   public:
    static void visitor(Method *m) {
      if (m->invocation_count() + m->compiled_invocation_count() >= 1 ) {
        m->clear_method_counters();
        m->clear_method_data();
      }
    }
  };

  SystemDictionary::methods_do(Inner::visitor);
}

static void print_method_bytecodes_inlined_counts() {

  class Inner {
   public:
    static void visitor(Method *m) {
      jlong count = m->bytecodes_inlined();
      if (count > 10) {
        ResourceMark rm;
        const char* name = m->name_and_sig_as_C_string();
        tty->print_cr("%s: %ld", name, count);
      }
    }
  };
...
static void count_mdos() {
    static int mdo_counter;
    class Inner {
     public:
      static void visitor(Method *m) {
...
static void zero_invoke_and_backedge_counters() {
  class Inner {
   public:
    static void visitor(Method *m) {
      if(m->method_data() != NULL) {
        m->method_data()->invocation_counter()->init();
        m->method_data()->backedge_counter()->init();
      }

      if(m->method_counters() != NULL) {
        m->method_counters()->invocation_counter()->init();
        m->method_counters()->backedge_counter()->init();
      }
    }
  };
...
void VM_PrintThreadTiming::doit() {
  class ThreadTimingClosure : public ThreadClosure {
    void do_thread(Thread *thread) {
      thread->print_and_update_timing();
    }
  };

  ThreadTimingClosure tc;
  {
    Threads::threads_do(&tc);
  }
}

// VM-op to load profiling data during a safepoint.
class VM_LoadProfilingData : public VM_Operation {
 public:
  VM_LoadProfilingData() { }

  VMOp_Type type() const { return VMOp_LoadProfilingData; }
  virtual void doit() {
    if(DumpAndReloadProfilingData) {
      Method::load_method_data_from_file("/tmp/hotspot_profiling");
    }
    else {
      Method::load_method_data_from_file(SerializedProfilingLoadLocation);
    }
  }
};

static uint64_t run_count = 0;
void InterpreterRuntime::handle_vm_indicator_method(JavaThread *thread) {
  CurrentIteration++;

  tty->print_cr("Methods Compiled Count: %d", CompileBroker::get_total_compile_count());
  tty->print_cr("Methods Deoptimized Count: %d", Deoptimization::total_deoptimization_count());
  tty->print_cr("Bytecodes Inlined Count: %d", CompileBroker::get_sum_bytecodes_inlined());
  tty->print_cr("Prof Data Lookup Count: %u", Method::attempted_lookup_prof_count());
  tty->print_cr("Prof Data Loaded Count: %u", Method::loaded_prof_count());

  if (PrintPerMethodBytecodesInlinedCounts) {
    print_method_bytecodes_inlined_counts();
  }

  {
    ThreadInVMfromJava tivmfj(thread);
    VM_PrintThreadTiming op;
    VMThread::execute(&op);
  }

  if(run_count++ == 0) {
    PreventMethodCompilation = 0;
    ...
    #if 0
    VM_LoadProfilingData vmop;
    VMThread::execute(&vmop);
    #endif

    if(DumpAndReloadProfilingData) {
        write_serialized_profiing_data("/tmp/hotspot_profiling");
    }

    if(ClearMethodCounters) {
    ...
    if(ZeroInvocationAndBackedgeCounters) {
      zero_invoke_and_backedge_counters();
    }

    count_mdos();

    if(SortC2CompilerQueue) {
      CompileBroker::c2_compile_queue()->sort_tasks_by_priority();
    }
    if(StartWithCompiliationDisabled) {
      CompilerThread::unpause();
    }
    if(WaitAtVmIndicatorForCompilerQueue) {
      while(!CompileBroker::c2_compile_queue()->is_empty()) {
        //sleep(1);
        // XXX This could safepoint. Is that desirable?
        os::sleep(Thread::current(), 1000, true);
      }
    }
  }
}
...
nmethod* InterpreterRuntime::frequency_counter_overflow(JavaThread* thread, address branch_bcp) {
  ...
  // Check whether this method should be compiled ever. The flag set here will
  // be checked in the interpreter to avoid making this call repeatedly.
  Method *m = thread->last_frame().interpreter_frame_method();
  if(m->has_vm_indicator()) {
    // Don't allow vm indicator method to compiler
    return NULL;
  }
  if(m->cmf_state() == Method::cmf_unchecked) {
    uint priority = Threads::compilable_methods_contains_priority(m);
    if(priority > 0) {
      m->set_cmf_state(Method::cmf_can_compile);
      m->set_comp_priority(priority);
    } else {
      if(!IgnoreCompilableMethodsList) {
        m->set_cmf_state(Method::cmf_cannot_compile);
        return NULL;
      } else {
        m->set_cmf_state(Method::cmf_can_compile);
        m->set_comp_priority(UINT_MAX);
      }
    }
  }
  ...
  {
    ThreadInVMfromJava tivfj(thread);
    MutexLockerEx locker(MethodDataSerialization_lock);
    if(ProfilingDataSerializer::shared()->is_open()) {
      ResourceMark rm;
      ProfilingDataSerializer::shared()->serialize_method(m);
    }
  }
...
IRT_ENTRY(void, InterpreterRuntime::update_mdp_for_ret(JavaThread* thread, int return_bci))
  ...
  assert(fr.is_interpreted_frame(), "must come from interpreter");
  Method *method = fr.interpreter_frame_method();
  MethodData* h_mdo = method->method_data();
  if(h_mdo == NULL) {
    method->set_method_data(method->method_data_old());
    h_mdo = method->method_data();
  }
  ...
  // XXX Needed to work around a the pointer on the stack being out of date.
  //ProfileData* data = h_mdo->data_at(h_mdo->dp_to_di(fr.interpreter_frame_mdp()));
  ProfileData* data = h_mdo->bci_to_data(fr.interpreter_frame_bci());
