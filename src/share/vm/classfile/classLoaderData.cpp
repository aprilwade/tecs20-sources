...
Klass* ClassLoaderDataGraph::lookup_klass_by_name(Symbol *name) {
  Thread *thread = Thread::current();
  for (ClassLoaderData* cld = _head; cld != NULL; cld = cld->next()) {
    oop loader = cld->class_loader();
    // TODO: Consider resolve_or_null instead
    Klass *k = SystemDictionary::find(name, Handle(loader), Handle(NULL), thread);
    if(k != NULL) {
      return k;
    }
  }
