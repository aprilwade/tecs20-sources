...
Method* ClassFileParser::parse_method(const ClassFileStream* const cfs,
  ...
  const u1 *user_annotation[4];
  int user_annotation_count = 0;
  ...
  while (method_attributes_count--) {
    ...
    if (method_attribute_name == vmSymbols::tag_code()) {
      ...
      if (method_attribute_name == vmSymbols::tag_signature()) {
        ...
        // Store user defined annotations in an array for later processing
        user_annotation[user_annotation_count++] = cfs->get_u1_buffer();
        cfs->guarantee_more(method_attribute_length, CHECK_NULL);
        cfs->skip_u1(method_attribute_length, CHECK_NULL);
      }
    } else {
      // Store user defined annotations in an array for later processing
  ...
  for(int i = 0; i < user_annotation_count; i++) {
    char *attr = (char*)user_annotation[i];
    if(attr == NULL || attr[0] == 0)
      continue;

    char *attr_split = strtok(attr, ":");
    if(attr_split != NULL && strcmp(attr_split, "VMIndicator") == 0) {
      puts("Found VMIndicator");
      m->set_has_vm_indicator(1);
    }
  }
