...
ciKlass *Method::set_pdp_klass(int index, int bci, ciKlass *val, const ciMethod *method) {
  if(val == NULL) {
    this->_pdps.push_back(PDP(index, bci, (uintptr_t)"NULL", Method::cimethod_name(method), pdp_type_str));
  } else {
    this->_pdps.push_back(PDP(index, bci, (uintptr_t)(strdup(val->get_Klass()->name()->as_C_string())), Method::cimethod_name(method), pdp_type_str));
  }
  return val;
}
