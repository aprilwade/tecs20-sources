...
static bool check_profiling_bounds() {
  return ProfileInterpreterBoundsRange > 0;
}

void InterpreterMacroAssembler::bound_check_profile(Register tmp, Label& bound_failure) {

  if(!check_profiling_bounds()) {
    return;
  }

  get_method(tmp);
  movptr(tmp, Address(tmp, Method::method_counters_offset()));
  testptr(tmp, tmp);
  jcc(Assembler::zero, bound_failure);

  movl(tmp, Address(tmp, MethodCounters::within_profiling_bounds_offset()));
  testl(tmp, tmp);
  jcc(Assembler::zero, bound_failure);
}
...
void InterpreterMacroAssembler::profile_arguments_type(Register mdp, Register callee, Register tmp, Register tmp2, bool is_virtual) {
  ...
  if (MethodData::profile_arguments() || MethodData::profile_return()) {
    ...
    if(check_profiling_bounds()) {
      Label bound_failure;
      movl(tmp2, 0);
      bound_check_profile(mdp, bound_failure);
      movl(tmp2, 1);
      bind(bound_failure);
    }
...
void InterpreterMacroAssembler::profile_return_type(Register mdp, Register ret, Register tmp) {
  ...
  if (ProfileInterpreter && MethodData::profile_return()) {
    ...
    bound_check_profile(mdp, profile_continue);
    test_method_data_pointer(mdp, profile_continue);
...
void InterpreterMacroAssembler::profile_parameters_type(Register mdp, Register tmp1, Register tmp2) {
  if (ProfileInterpreter && MethodData::profile_parameters()) {
    ...
    bound_check_profile(tmp1, profile_continue);
    test_method_data_pointer(mdp, profile_continue);
...
void InterpreterMacroAssembler::profile_ret(Register return_bci, Register mdp, Register tmp) {
  if (ProfileInterpreter) {
    Label profile_continue;
    uint row;
...
void InterpreterMacroAssembler::profile_switch_case(Register index,
  ...
  if (ProfileInterpreter) {
    ...
    Label bound_failure;
    bound_check_profile(reg2, bound_failure);
