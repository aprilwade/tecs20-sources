...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
  ...
  if (UseLoopCounter) {
    ...
    if (TieredCompilation) {
      ...
      if(ProfileInterpreterBoundsRange > 0) {
        ...
        __ cmp32(rax, Address(rcx, MethodCounters::profiling_upper_bound_offset()));
        __ jcc(Assembler::greater, bound_failure);

        __ cmp32(rax, Address(rcx, MethodCounters::profiling_lower_bound_offset()));
        __ jcc(Assembler::less, bound_failure);

        __ movl(Address(rcx, MethodCounters::within_profiling_bounds_offset()), 1);
        __ jmp(bound_done);

        __ bind(bound_failure);
        __ movl(Address(rcx, MethodCounters::within_profiling_bounds_offset()), 0);
  ...
  if (UseLoopCounter) {
    ...
    if (UseOnStackReplacement) {
     ...
     if(CompileOnlyMethodsWithLoadedProfile || CompilableMethodsListLocation != NULL) {
        // Get the method pointer for one or both of the next couple of checks
        __ get_method(rcx);
      }
      if(CompileOnlyMethodsWithLoadedProfile) {
        __ cmpb(Address(rcx, Method::has_loaded_profiling_data_offset()), 0);
        __ jcc(Assembler::equal, dispatch);
      }
      if(CompilableMethodsListLocation != NULL) {
        __ cmpb(Address(rcx, Method::cmf_state_offset()), Method::cmf_cannot_compile);
        __ jcc(Assembler::equal, dispatch);
      }
...
void TemplateTable::invokevirtual_helper(Register index,
  ...
  __ profile_arguments_type(rax, method, rbcp, rdx, true);
  ...
  __ profile_arguments_type(rdx, method, rbcp, rax, true);
...
void TemplateTable::invokespecial(int byte_no) {
  ...
  // do the call
  __ profile_call(rax);
  __ profile_arguments_type(rax, rbx, rbcp, rdx, false);
...
void TemplateTable::invokeinterface(int byte_no) {
  ...
  __ profile_arguments_type(rdx, rbx, rbcp, rdx, true);
...
void TemplateTable::invokehandle(int byte_no) {
  ...
  __ profile_final_call(rax);
  __ profile_arguments_type(rdx, rbx_method, rbcp, rax, true);
...
void TemplateTable::invokedynamic(int byte_no) {
  ...
  __ profile_arguments_type(rdx, rbx_method, rbcp, rcx, false);
