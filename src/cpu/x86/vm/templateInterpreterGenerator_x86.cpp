...
void TemplateInterpreterGenerator::generate_counter_incr(
  ...
  // Check if this method has the VMIndicator annotation and call the handler if so.
  Label skip_vm_indicator;
  __ cmpl(Address(rbx, Method::vm_indicator_offset()), 0);
  __ jcc(Assembler::equal, skip_vm_indicator);
  __ call_VM(noreg,  CAST_FROM_FN_PTR(address, InterpreterRuntime::handle_vm_indicator_method));
  __ bind(skip_vm_indicator);

  if(AllocMethodDataOnInvocation) {
    __ movptr(rax, Address(rbx, Method::method_data_offset()));
    __ testptr(rax, rax);
    Label has_mdo;
    __ jcc(Assembler::notZero, has_mdo);
    __ call_VM(noreg, CAST_FROM_FN_PTR(address, Method::interp_alloc_mdo));
    __ bind(has_mdo);
  }
  ...
  if (TieredCompilation) {
    ...
    if(ProfileInterpreterBoundsRange > 0) {
      __ movptr(rax, Address(rbx, Method::method_counters_offset()));

      Label bound_failure, bound_done;
      __ cmp32(rcx, Address(rax, MethodCounters::profiling_upper_bound_offset()));
      __ jcc(Assembler::greater, bound_failure);

      __ cmp32(rcx, Address(rax, MethodCounters::profiling_lower_bound_offset()));
      __ jcc(Assembler::less, bound_failure);

      __ movl(Address(rax, MethodCounters::within_profiling_bounds_offset()), 1);
      __ jmp(bound_done);

      __ bind(bound_failure);
      __ movl(Address(rax, MethodCounters::within_profiling_bounds_offset()), 0);
...
void TemplateInterpreterGenerator::generate_counter_overflow(Label& do_continue) {
  ...
  __ cmp8(ExternalAddress((address)&PreventMethodCompilation), 0);
  __ jcc(Assembler::notEqual, do_continue);
  if(CompileOnlyMethodsWithLoadedProfile) {
    __ cmpb(Address(rbx, Method::has_loaded_profiling_data_offset()), 0);
    __ jcc(Assembler::equal, do_continue);
  }
  if(CompilableMethodsListLocation != NULL) {
    __ cmpb(Address(rbx, Method::cmf_state_offset()), Method::cmf_cannot_compile);
    __ jcc(Assembler::equal, do_continue);
  }
