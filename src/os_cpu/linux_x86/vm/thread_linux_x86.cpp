...
void Thread::timing(double *user, double *sys) {
  ...
  sprintf(filename_buf, "/proc/%d/task/%d/stat", pid, this->osthread()->thread_id());
  FILE *f = fopen(filename_buf, "r");
  ...
  s++;
  sscanf(s, " %c %d %d %d %d %d %lu %lu %lu %lu %lu %lu %lu",
  ...
  while(1) {
    int c = fgetc(f);
    assert(c != -1, "/proc stat wrong format?");
